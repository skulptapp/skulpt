...
Code is not the only useful contribution. Documentation, translations, accessibility improvements, exercise corrections, tests, and reproducible device reports are welcome.
...
This gives maintainers and contributors a chance to agree on the problem and constraints before a large pull request exists.
...
Documentation and isolated unit-test changes may not need a native build. State which checks you did and did not run in the pull request.
...
State which permissions and data sources the change uses. Test denied permission and unavailable-service paths. Review whether imported rows can enter SyncLayer and whether the store privacy declarations need an update.

Never include real health samples in a test fixture, screenshot, log, or issue.

### Native changes

For changes under `modules/`, `targets/`, `app.config.js`, or native permissions, name the physical device, simulator, emulator, OS version, and Watch setup used for testing.
...
- automated and manual checks actually run;
...
The roadmap in [README.md](README.md#roadmap) defines the current product direction. A contribution can propose a change to it, but should not silently replace or contradict it.
